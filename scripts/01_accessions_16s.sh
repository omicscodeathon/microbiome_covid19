...
# This script grabs the accession ids that correspond to a given country in the Metadata file and writes it to a new .txt file.
...
head -n $accessions_plus_header $input_file | tail -n $num_accessions | cut -f$field_num -d',' > $output_file
